...
func (b *Blockstore) getImpl(key []byte, handler func(val []byte) error) error {
	b.oplock.RLock()
...
func (b *Blockstore) Has(cid cid.Cid) (bool, error) {
	err := b.getImpl(b.opts.CidToKey(cid), func(val []byte) error { return nil })
...
func (b *Blockstore) Get(key cid.Cid) (blocks.Block, error) {
	...
	err := b.getImpl(b.opts.CidToKey(key), func(v []byte) (err error) {
		if b.rehash.Load() {
			var key2 cid.Cid
...
func (b *Blockstore) View(cid cid.Cid, callback func([]byte) error) error {
	err := b.getImpl(b.opts.CidToKey(cid), callback)
	if lmdb.IsNotFound(err) || lmdb.IsErrno(err, lmdb.BadValSize) {
		// lmdb returns badvalsize with nil keys.
...
func (b *Blockstore) GetSize(cid cid.Cid) (int, error) {
	size := -1
	err := b.getImpl(b.opts.CidToKey(cid), func(v []byte) error {
		size = len(v)
		return nil
	})
	if lmdb.IsNotFound(err) || lmdb.IsErrno(err, lmdb.BadValSize) {
...
func (b *Blockstore) Put(block blocks.Block) error {
	return b.updateImpl(func(txn *lmdb.Txn) error {
		...
		err := txn.Put(b.db, key, block.RawData(), lmdb.NoOverwrite)
...
type DataStorage interface {
	...
	PutData(key []byte, valueF func(prevVal []byte, exists bool) (newVal []byte, newExists bool, err error)) error
}

// GetData retrieves arbitrary key-value pair from the blockstore
// 	This is useful to store some meta data in the storage
func (b *Blockstore) GetData(key []byte) ([]byte, error) {
	var res []byte
...
func (b *Blockstore) PutData(key []byte, valueF func(prevVal []byte, exists bool) (newVal []byte, newExists bool, err error)) error {
	return b.updateImpl(func(txn *lmdb.Txn) error {
		var err error
		prev, err := txn.Get(b.db, key)
