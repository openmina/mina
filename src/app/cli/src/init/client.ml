  ...
  Command.async
    ...
    Deferred.ignore_m
      (let _logger = Logger.null () in
       ...
       let block = External_transition.decompose raw_transition in

       (* let proto_state = External_transition.Validated.Stable.V1 _transition. *)
       ...
       let proto_state = block |> Mina_block.header |> Header.protocol_state in
       (* let h = proto_state |> Protocol_state.constants |> Hashable.Binable. Protocol_constants_checked.Value.hash in
          printf "\nconstants_hash: %s\n" h ; *)
