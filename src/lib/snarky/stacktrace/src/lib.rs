...
use std::fmt::Write;
use ocaml_interop::{OCaml, OCamlRef, OCamlInt, ToOCaml};

ocaml_interop::ocaml_export! {
    fn rust_maybe_save_stacktrace(
        rt,
        _depth: OCamlRef<OCamlInt>,
    ) -> OCaml<Option<String>> {
        let path = "/tmp/SAVE_STACKTRACE";

        let res = if std::path::Path::new(path).exists() {
            // eprintln!("stacktrace=\n{}", short_stacktrace());
            Some(short_stacktrace())
        } else {
            None
        };
...
pub fn short_stacktrace() -> String {
    ...
    for (index, (filename, name, line)) in bt
        ...
        .filter(|(_, (filename, name, _))| {
            return !(filename
                .map(|filename| {
                    filename.ends_with("stacktrace/src/lib.rs")
                        || filename
                            .components()
